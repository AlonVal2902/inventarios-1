...
export function EntradasTemplate() {
  ...
  const avanzarPagina = () => {
    const maxPaginas = Math.ceil(entradasOrdenadas.length / registrosPorPagina);
  ...
  const registrosActuales = entradasOrdenadas.slice(inicio, fin);
  const maxPaginas = Math.ceil(entradasOrdenadas.length / registrosPorPagina);
  ...
  useEffect(() => {
    const fetchentradas = async () => {
      try {
        const response = await entradasApi.findAll();
  ...
  const buscarProducto = async (idproducto) => {
    if (!idproducto) {
      console.warn("Código es requerido para buscar un producto.");
      return;
    }
  ...
  const handleRegister = async () => {
    const { idproducto, descripcion, cantidad, empaquetado, diseño } = formData;

    if (!idproducto || !cantidad || !empaquetado) {
      alert("Complete con todos los campos requeridos.");
      return;
    }
    ...
    try {
      ...
      try {
        productoExistente = await inventarioApi.findOne(idproducto);
      } catch (error) {
        console.warn("No se encontró el producto en  el inventario:", error);
      }

      if (!productoExistente) {
        // Verificar si el producto existe en la tabla de productos
        const productoEnProductos = await productosApi.findOne(idproducto);
        if (!productoEnProductos || !productoEnProductos.data) {
          alert("El producto no existe en la tabla de productos.");
          return;
        }
      }

      // Si el producto ya existe, registramos la entrada normalmente
      await entradasApi.create(newEntrada);
      alert("Producto registrado");

      setentradas((prev) => [...prev, newEntrada]);
      clearForm();

      // Sumamos al inventario existente
      await sumarInventario(idproducto);
    } catch (error) {
  ...
  const sumarInventario = async (idproducto) => {
    try {
      ...
      if (inventario && inventario.data) {
        ...
        const cantidadSumar = parseInt(formData.cantidad, 10) || 0;
        const nuevaCantidad = cantidadActual + cantidadSumar;
        ...
        alert("Inventorio actualizado");
      } else {
        alert("Producto no encontrado en el inventario.");
      }
    } catch (error) {
      console.error("Error actualizando el inventorio:", error);
  ...
  const openDetails = (entrada) => setSelectedProduct(entrada);
    ...
    <Container>
      ...
      <Form>
        ...
        <Table style={{ marginTop: "50px" }}>
          ...
          >
            <tr>
              ...
              <th style={{ padding: "70 160px" }}>Vistas</th>
            </tr>
          </thead>
          <tbody>
            {registrosActuales.length === 0 ? (
              ...
              registrosActuales.map((entrada, index) => (
                <tr key={index}>
                  <td style={{ padding: "70 140px" }}>{entrada.descripcion}</td>
        ...
        <div style={{ textAlign: "center" }}>
          <ButtonStyle onClick={retrocederPagina} disabled={paginaActual === 0}>
            Anterior
          </ButtonStyle>
